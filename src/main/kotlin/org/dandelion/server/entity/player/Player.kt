package org.dandelion.server.entity.player
...
import org.dandelion.server.events.BlockFace
import org.dandelion.server.events.ClickAction
import org.dandelion.server.events.MouseButton
import org.dandelion.server.events.PlayerBlockClickEvent
import org.dandelion.server.events.PlayerBlockInteractionEvent
import org.dandelion.server.events.PlayerBlockListSelectedEvent
import org.dandelion.server.events.PlayerBlockListToggledEvent
import org.dandelion.server.events.PlayerChangeLevel
import org.dandelion.server.events.PlayerEntityClickEvent
import org.dandelion.server.events.PlayerLevelSavedEvent
import org.dandelion.server.events.PlayerMoveEvent
import org.dandelion.server.events.PlayerPressEvent
import org.dandelion.server.events.PlayerReleaseEvent
import org.dandelion.server.events.PlayerRespawnedEvent
import org.dandelion.server.events.PlayerSendMessageEvent
import org.dandelion.server.events.PlayerSpawnUpdatedEvent
import org.dandelion.server.events.PlayerTexturePackChangedEvent
import org.dandelion.server.events.PlayerTexturePromptRespondedEvent
...
import org.dandelion.server.network.packets.cpe.client.ClientNotifyPositionAction
import org.dandelion.server.network.packets.cpe.client.ClientPlayerClick
import org.dandelion.server.network.packets.cpe.server.ServerCinematicGui
import org.dandelion.server.network.packets.cpe.server.ServerClickDistance
import org.dandelion.server.network.packets.cpe.server.ServerExtEntityTeleport
import org.dandelion.server.network.packets.cpe.server.ServerHackControl
import org.dandelion.server.network.packets.cpe.server.ServerHoldThis
import org.dandelion.server.network.packets.cpe.server.ServerMakeSelection
import org.dandelion.server.network.packets.cpe.server.ServerRemoveSelection
import org.dandelion.server.network.packets.cpe.server.ServerSetBlockPermission
import org.dandelion.server.network.packets.cpe.server.ServerSetHotbar
import org.dandelion.server.network.packets.cpe.server.ServerSetSpawnpoint
import org.dandelion.server.network.packets.cpe.server.ServerSetTextHotKey
import org.dandelion.server.network.packets.cpe.server.ServerToggleBlockList
import org.dandelion.server.network.packets.cpe.server.ServerVelocityControl
...
class Player(
    ...
    override val permissions: Map<String, Boolean>
        get() = PermissionRepository.getAllPermissionEntries(name)
    ...
    override fun interactWithBlock(
        ...
        if (!hasPermission("dandelion.level.build.${currentLevel.id}", true)){
            sendMessage(MessageRegistry.Server.Level.getNoBuildPermission())
            ServerSetBlock(
                x,
                y,
                z,
