...
/**
 * this defines a plugin group by dependency, these will be loaded in the smae classloader
 */
private data class PluginCluster(
    val plugins: Set<String>, //the plugins name
    val classLoader: PluginClassLoader,
...
internal object PluginRegistry {
    ...
    private const val INFO_PATH = "plugin.yml"
    fun init(){
    ...
    //resolves the plugins that depends in other, so they will be loaded in the same classloader to avoid classloaderIsolation
    ...
    //find the cluster of dependents plugins
    ...
    //loads a cluster on the same classloader
    private fun loadPluginCluster(cluster: Set<String>, discoveredPlugins: List<DiscoveredPlugin>){
        ...
        orderedCluster.forEach { discoveredPlugin ->
            try {
                ...
                loadedPlugins[discoveredPlugin.info.name] = loadedPlugin
    ...
    private fun initializeAllPlugins(){
        ...
        initializationOrder.forEach { pluginName ->
            val loadedPlugin = loadedPlugins[pluginName]
    ...
    fun getPlugin(name: String): LoadedPlugin? {
        return loadedPlugins.entries.find { it.key.equals(name, ignoreCase = true) }?.value
    }
    fun getAllPlugins(): Map<String, LoadedPlugin> {
        return loadedPlugins.toMap()
    }
    fun loadPluginByName(name: String): Boolean {
        if (loadedPlugins.keys.any { it.equals(name, ignoreCase = true) }) {
        ...
        val pluginInfo = try { readInfo(jarFile) } catch (e: Exception) {
        ...
        val pluginInstance = try { loadPluginInstance(discoveredPlugin, classLoader) } catch (e: Exception) {
        ...
        loadedPlugins[pluginInfo.name] = loadedPlugin
        try {
            pluginInstance.init()
        } catch (e: Exception) {
            Console.errLog("Error initializing plugin '$name': ${e.message}")
            loadedPlugins.remove(pluginInfo.name)
            classLoader.close()
            return false
        }
        Console.log("Plugin '$name' loaded dynamically.")
        return true
    }

    fun unloadPluginByName(name: String): Boolean {
        val loadedEntry = loadedPlugins.entries.find { it.key.equals(name, ignoreCase = true) }
        ...
        try {
            ...
            Console.errLog("Error shutting down plugin '$name': ${e.message}")
        }
        loadedPlugins.remove(loadedEntry.key)
        try {
            loaded.classLoader.close()
        } catch (e: Exception) {
            Console.errLog("Error closing classloader for plugin '$name': ${e.message}")
        }
        Console.log("Plugin '$name' unloaded dynamically.")
        return true
    }
