...
object Server {
    ...
    fun reloadConfig() {
        ...
        ServerInfo.maxPlayers =
            config.getInt("server.max-players", ServerInfo.maxPlayers)
        ServerInfo.isPublic =
            config.getBoolean("server.public", ServerInfo.isPublic)
        ServerInfo.verifyUsers =
            config.getBoolean("server.verify-users", ServerInfo.verifyUsers)
        ServerInfo.debugMode =
            config.getBoolean("server.debug-mode", ServerInfo.debugMode)

        ServerInfo.defaultLevel =
            config.getString("level.default-level", ServerInfo.defaultLevel)
        ServerInfo.autoSaveInterval =
            config
                .getInt(
                    "level.auto-save-interval",
                    ServerInfo.autoSaveInterval.inWholeSeconds.toInt(),
                )
                .seconds

        ServerInfo.heartbeatEnabled =
            config.getBoolean("heartbeat.enabled", ServerInfo.heartbeatEnabled)
        ServerInfo.heartbeatUrl =
            config.getString("heartbeat.url", ServerInfo.heartbeatUrl)
        ServerInfo.heartbeatData =
            config.getString("heartbeat.data", ServerInfo.heartbeatData)
        ServerInfo.heartbeatInterval =
            config
                .getInt(
                    "heartbeat.interval",
                    ServerInfo.heartbeatInterval.inWholeSeconds.toInt(),
                )
                .seconds
    }
