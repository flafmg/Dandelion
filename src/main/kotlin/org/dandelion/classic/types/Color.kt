...
data class Color(
    ...
    constructor(hex: String) : this(
        ...
        require(isValidHex(hex)) { "Invalid hex color format: $hex. Expected format: #RRGGBB or RRGGBB" }
    }
    override fun toString(): String {
        return "Color(r=$red, g=$green, b=$blue, hex=#%02X%02X%02X)".format(red.toInt(), green.toInt(), blue.toInt())
    }

    companion object {
        private fun isValidHex(hex: String): Boolean {
            val cleanHex = hex.removePrefix("#")
            return cleanHex.length == 6 && cleanHex.all { it.isDigit() || it.lowercaseChar() in 'a'..'f' }
        }
