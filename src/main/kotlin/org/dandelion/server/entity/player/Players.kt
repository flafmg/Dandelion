...
import org.dandelion.server.events.PlayerConnectEvent
...
import org.dandelion.server.network.packets.classic.server.ServerIdentification
import org.dandelion.server.permission.Group
...
object Players {
    ...
    fun broadcastMessage(message: String, messageTypeId: Byte = 0x00) {
        getAll().forEach { it.sendMessage(message, messageTypeId) }
    }

    fun kickAll(reason: String = "Server maintenance") {
        getAll().forEach { it.kick(reason) }
    }
