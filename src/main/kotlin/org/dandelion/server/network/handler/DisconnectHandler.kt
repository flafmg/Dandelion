...
class DisconnectHandler : ChannelInboundHandlerAdapter() {
    ...
    override fun channelInactive(ctx: ChannelHandlerContext) {
        ...
        try {
            ...
            Console.errLog(
                "Error handling disconnection for $remoteAddress: ${e.message}"
            )
    ...
    override fun exceptionCaught(ctx: ChannelHandlerContext, cause: Throwable) {
        ...
        Console.errLog(
            "Exception caught for client $remoteAddress: ${cause.message}"
        )
        ...
        try {
            ...
            Console.errLog(
                "Error during exception handling for $remoteAddress: ${e.message}"
            )
            forceDisconnection(ctx.channel())
        } finally {
    ...
    override fun channelRegistered(ctx: ChannelHandlerContext) {
        Console.debugLog(
            "New client connected: ${ctx.channel().remoteAddress()}"
        )
    ...
    override fun channelUnregistered(ctx: ChannelHandlerContext) {
        ...
        try {
            ...
            Console.errLog(
                "Error during channel unregistered cleanup for $remoteAddress: ${e.message}"
            )
            forceDisconnection(ctx.channel())
        } finally {
