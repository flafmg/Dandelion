...
import org.dandelion.server.commands.annotations.*
...
internal object CommandProcessor {
    fun processCommand(command: Command): CommandInfo? {
        ...
        if (
            !org.dandelion.server.commands.model.Command::class
                .java
                .isAssignableFrom(clazz)
        ) {
            Console.errLog(
                "Could not register command '${clazz.name}', it doesnt implement Command interface"
            )
        }
        if (commandDefAnnotation == null) {
            Console.errLog(
                "Could not register command '${clazz.name}', it doesnt have @Command annotation"
            )
            return null
        }
        ...
        val executeMethods =
            clazz.declaredMethods.filter {
                it.isAnnotationPresent(OnExecute::class.java)
            }
        if (executeMethods.size > 1) {
            Console.errLog(
                "Could not register command '${clazz.name}', it contains more than one @OnExecute"
            )
        }
        ...
        val permission =
            executeMethod
                ?.getAnnotation(RequirePermission::class.java)
                ?.permission ?: ""
    ...
    private fun processSubCommands(
        methods: List<Method>,
        subCommands: MutableMap<String, SubCommandInfo>,
        path: List<String>,
    ) {
        methods
            .filter { it.isAnnotationPresent(OnSubCommand::class.java) }
            .forEach { method ->
                ...
                annotation.aliases.forEach { alias ->
                    ...
                    subCommands[aliasKey] =
                        subCommandInfo.copy(path = aliasPath)
                }
            }
    }
    ...
    fun executeCommand(
        ...
        if (args.isNotEmpty()) {
            ...
            if (subCommandPath != null) {
                val remainingArgs =
                    args.drop(subCommandPath.path.size).toTypedArray()
                return executeMethod(
                    subCommandPath.method,
                    commandInfo.instance,
                    executor,
                    remainingArgs,
                    subCommandPath.permission,
                )
            }
        }
        ...
        executor.sendMessage(
            "&cCommand not found. Available subcommands: ${commandInfo.subCommands.keys.joinToString(", ")}"
        )
        return false
    }
    ...
    private fun processReferSelf(
        method: Method,
        executor: CommandExecutor,
        args: Array<String>,
    ): Array<String> {
        val referSelf =
            method.getAnnotation(ReferSelf::class.java) ?: return args
    ...
    private fun validateExecutorType(
        method: Method,
        executor: CommandExecutor,
    ): Boolean {
        if (
            method.isAnnotationPresent(RequirePlayer::class.java) &&
                executor !is Player
        ) {
            executor.sendMessage(
        ...
        if (
            method.isAnnotationPresent(RequireConsole::class.java) &&
                executor !is Console
        ) {
            executor.sendMessage(
                "&cThis command can only be executed by console"
            )
            return false
        }
        return true
        ...
        if (argRange != null) {
            when {
                args.size < argRange.min -> {
                    executor.sendMessage(
                        "&cThis command expects at least ${argRange.min} arguments"
                    )
                    return false
                }
                args.size > argRange.max -> {
                    executor.sendMessage(
                        "&cThis command expects at most ${argRange.max} arguments"
                    )
                    return false
                }
            }
