...
class PermissionCommand : Command {
    ...
    @RequirePermission("dandelion.command.permission.view")
    fun groupCommand(executor: CommandExecutor, args: Array<String>) {
        ...
        when (args[0].lowercase()) {
            "create", "c", "add", "new" -> handleGroupCreate(executor, args.drop(1).toTypedArray())
            "delete", "del", "d", "remove", "rem" -> handleGroupDelete(executor, args.drop(1).toTypedArray())
            "list", "l", "ls" -> handleGroupList(executor, args.drop(1).toTypedArray())
            "info", "i", "show" -> handleGroupInfo(executor, args.drop(1).toTypedArray())
            "setpriority", "priority", "setprio", "prio" ->
                handleGroupSetPriority(executor, args.drop(1).toTypedArray())
            "setdisplay", "display", "setname", "name" ->
            ...
            "removepermission", "remove", "rem", "rp", "delperm" ->
                handleGroupRemovePermissionEntry(
                    executor,
                    args.drop(1).toTypedArray(),
                )
            "listperms", "listpermissions", "perms", "lp" ->
    ...
    fun playerCommand(executor: CommandExecutor, args: Array<String>) {
        ...
        when (args[0].lowercase()) {
            "info", "i", "show" -> handlePlayerInfo(executor, args.drop(1).toTypedArray())
            "addgroup", "ag", "addg", "agroup" ->
                handlePlayerAddGroup(executor, args.drop(1).toTypedArray())
            "removegroup", "remgroup", "rg", "rgroup", "delgroup", "remg" ->
                handlePlayerRemoveGroup(executor, args.drop(1).toTypedArray())
            "setgroups", "sgroups", "setg" ->
                handlePlayerSetGroups(executor, args.drop(1).toTypedArray())
            "listgroups", "lgroups", "lg", "groups" ->
            ...
            "listperms", "listpermissions", "list", "lp", "perms" ->
                handlePlayerListPermissions(
                    executor,
                    args.drop(1).toTypedArray(),
                )
            "listownperms", "lop", "ownperms", "own" ->
    ...
    @RequirePermission("dandelion.command.permission.reload")
    ...
    private fun showGroupHelp(executor: CommandExecutor) {
        ...
        if (executor.hasPermission("dandelion.command.permission.group.create")) {
            MessageRegistry.Commands.Permission.Help.sendGroupCreate(executor)
        }
        if (executor.hasPermission("dandelion.command.permission.group.delete")) {
        ...
        if (executor.hasPermission("dandelion.command.permission.group.edit")) {
    ...
    private fun showPlayerHelp(executor: CommandExecutor) {
        ...
        if (executor.hasPermission("dandelion.command.permission.player.groups")) {
        ...
        if (executor.hasPermission("dandelion.command.permission.player.perms")) {
    ...
    private fun handlePlayerListPermissions(
        ...
        val playerName = args[0]
        val allPermissions = PermissionRepository.getAllPermissionEntries(playerName)
        ...
        allPermissions.toSortedMap().forEach { (perm, hasPermission) ->
            val player = PermissionRepository.getPlayer(playerName)
