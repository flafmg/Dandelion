...
import org.dandelion.server.plugins.model.Plugin
import org.dandelion.server.plugins.model.PluginInfo
...
private data class DiscoveredPlugin(val info: PluginInfo, val jarFile: File)
...
internal object PluginRegistry {
    ...
    fun init() {
        ...
        if (!pluginsDir.exists()) {
            pluginsDir.mkdirs()
            Console.log(
                "'plugins' folder created in: ${pluginsDir.absolutePath}"
            )
            return
        }
        ...
        pluginsDir
            ...
            ?.forEach { jarFile ->
                try {
                    ...
                    Console.log(
                        "Plugin found: '${pluginInfo.name}' version: '${pluginInfo.version}'"
                    )
                } catch (e: Exception) {
                    Console.errLog(
                        "Error loading plugin '${jarFile.name}': ${e.message}"
                    )
                }
            }
        return discovered
    ...
    private fun readInfo(jarFile: File): PluginInfo {
        JarFile(jarFile).use { jar ->
            val entry =
                jar.getJarEntry(INFO_PATH)
                    ?: throw IllegalArgumentException(
                        "plugin.yml not found for ${jarFile.name}"
                    )
            val config = YamlConfig.load(jar.getInputStream(entry))
            val name =
                config.getString("name")
                    ?: throw java.lang.IllegalArgumentException(
                        "Field 'name' not found for ${jarFile.name}"
                    )
            val version = config.getString("version", "1.0.0")
            val description = config.getString("description", "none")
            val authors =
                when {
                    config.getStringList("authors") != null ->
                        config.getStringList("authors")!!
                    config.getString("author") != null ->
                        listOf(config.getString("author")!!)
                    else -> emptyList()
                }
    ...
    private fun resolveDependencyClusters(
        ...
        plugins.forEach { plugin ->
            if (plugin.info.name !in visited) {
                val group =
                    findCluster(plugin.info.name, pluginMap, mutableSetOf())
                groups.add(group)
        ...
        plugins.forEach { plugin ->
            plugin.info.dependencies.forEach { (depName, depVersion) ->
                if (depName !in pluginMap) {
                    val versionInfo =
                        if (depVersion != null) " (version: $depVersion)"
                        else " (any version)"
    ...
    private fun detectCircularDependencies(
        ...
        fun dfs(pluginName: String): Boolean {
            if (pluginName in gray) return true
            if (pluginName in black) return false
            white.remove(pluginName)
            gray.add(pluginName)
            pluginMap[pluginName]?.info?.dependencies?.forEach { (depName, _) ->
                if (dfs(depName)) {
                    throw IllegalStateException(
                        "Circular dependency betwen: $pluginName -> $depName"
                    )
                }
            }
    ...
    private fun findCluster(
        ...
        if (pluginName in visited) return emptySet()
        ...
        pluginMap.values.forEach { other ->
            if (other.info.dependencies.any { it.first == pluginName }) {
                connected.addAll(
                    findCluster(other.info.name, pluginMap, visited)
                )
            }
        }
        return connected
    ...
    private fun loadPluginCluster(
        cluster: Set<String>,
        discoveredPlugins: List<DiscoveredPlugin>,
    ) {
        val clusterPlugins =
            discoveredPlugins.filter { it.info.name in cluster }
        ...
        orderedCluster.forEach { discoveredPlugin ->
            try {
                ...
                loadedPlugins[discoveredPlugin.info.name.lowercase()] =
                    loadedPlugin
                Console.log(
                    "Plugin '${discoveredPlugin.info.name}' instantiated"
                )
            } catch (e: Exception) {
                Console.errLog(
                    "Could not instantiate plugin '${discoveredPlugin.info.name}': ${e.message}"
                )
                throw e
            }
        }
    ...
    private fun orderPluginCluster(
        plugins: List<DiscoveredPlugin>
        ...
        fun visit(pluginName: String) {
            if (pluginName in visited) return
            visited.add(pluginName)
            val plugin = pluginMap[pluginName] ?: return
            plugin.info.dependencies.forEach { (depName, _) -> visit(depName) }
            ordered.add(plugin)
        }
    ...
    private fun loadPluginInstance(
        ...
        if (instance !is Plugin) {
            throw IllegalArgumentException(
                "Main class for ${discoveredPlugin.info.name} does not implement the 'Plugin' interface."
            )
        }
        return instance
    }
        ...
        initializationOrder.forEach { pluginName ->
            ...
            if (loadedPlugin != null) {
                try {
                    ...
                    Console.errLog(
                        "Error initializing plugin '${loadedPlugin.info.name}': ${e.message}"
                    )
                    e.printStackTrace()
                }
            }
        ...
        shutdownOrder.forEach { pluginName ->
            ...
            if (loadedPlugin != null) {
                try {
                    ...
                    Console.errLog(
                        "Error shutting plugin down '${loadedPlugin.info.name}': ${e.message}"
                    )
                    e.printStackTrace()
                }
            }
    ...
    private fun getInitializationOrder(): List<String> {
        ...
        fun visit(pluginName: String) {
            if (pluginName in visiting) {
                throw IllegalStateException(
                    "Circular dependency on initialization: $pluginName"
                )
            }
            if (pluginName in visited) return
    ...
    fun loadPluginByName(name: String): Boolean {
        ...
        val jarFile =
            pluginsDir
                .listFiles { file ->
                    file.extension == "jar" &&
                        file.nameWithoutExtension.equals(name, true)
                }
                ?.firstOrNull()
        if (jarFile == null) {
            Console.errLog(
                "Plugin jar for '$name' not found in plugins directory."
            )
            return false
        }
        val pluginInfo =
            try {
                ...
                Console.errLog(
                    "Error reading plugin info for '$name': ${e.message}"
                )
                return false
            }
        val classLoader =
        ...
        val pluginInstance =
            try {
                ...
                Console.errLog(
                    "Error loading plugin instance for '$name': ${e.message}"
                )
                classLoader.close()
                return false
            }
        ...
        try {
            ...
            Console.errLog(
                "Error initializing plugin '${pluginInfo.name}': ${e.message}"
            )
    ...
    fun unloadPluginByName(name: String): Boolean {
        val normalizedName = name.lowercase()
        val loadedEntry =
            loadedPlugins.entries.find { it.key == normalizedName }
        ...
        val pluginsToUnload =
            loadedPlugins
                .filter { it.value.classLoader === targetClassLoader }
                .toList()

        pluginsToUnload.forEach { (key, pluginToUnload) ->
            try {
                Console.log(
                    "Shutting down '${pluginToUnload.info.name}' (part of unload cluster)"
                )
                pluginToUnload.instance.shutdown()
            } catch (e: Exception) {
                Console.errLog(
                    "Error shutting down plugin '${pluginToUnload.info.name}': ${e.message}"
                )
            }
        ...
        try {
            ...
            Console.errLog(
                "Error closing classloader for cluster containing '$name': ${e.message}"
            )
        }

        Console.log(
            "Plugin(s) '${pluginsToUnload.joinToString(", ") { it.second.info.name }}' unloaded dynamically (cluster unload)."
        )
        return true
    }
    ...
    fun reloadPluginByName(name: String): Boolean {
        ...
        if (!loadSuccess) {
            Console.errLog(
                "Failed to load plugin '$jarFileName' after unloading '$name'. Plugin is currently not loaded."
            )
            return false
        }
